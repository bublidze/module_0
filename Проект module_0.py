...
def guessing_the_number(guess_number):
    '''Функция принимает загаданное значение рандомного числа,
    после чего методом бинарного поиска отгадывает его значение
    ...
    return (count_attempts)  # Число угадано, выход из цикла. Возврат 
                             # кол-ва попыток.


def score_game(guessing_the_number):
    '''Запускаем игру 1000 раз, чтобы узнать среднее кол-во угадываний 
    одного числа'''
    random_count_list = []
    np.random.seed(1)  # Фиксируем RANDOM SEED, дабы эксперимент был 
                       # воспроизводим!
